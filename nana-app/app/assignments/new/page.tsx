    ...
    try {
      ...
      for (let i = 0; i < lines.length; i += 4) {
        ...
        try {
          ...
          const dateTimeMatch = deadlineText.match(
          ...
          setError(
            `${Math.floor(i / 4) + 1}番目の課題の日付解析エラー: ${dateError instanceof Error ? dateError.message : '不明なエラー'}`
          )
          setLoading(false)
          return
        }
        ...
        if (
          category.includes('小テスト') ||
          category.includes('テスト') ||
          category.includes('試験')
        ) {
      ...
      if (assignments.length === 0) {
        setError(
          '有効な課題データが見つかりませんでした。正しいフォーマットで入力してください。'
        )
        setLoading(false)
        return
      }
    ...
    try {
      for (const assignment of parsedAssignments) {
        ...
        const existingCourse = courses.find(
          c => c.name === assignment.subject || c.code === assignment.subject
        )
